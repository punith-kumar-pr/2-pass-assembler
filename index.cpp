...
void disect(char line[], struct state *s){       //disect - scan 
    int i, j=0, k=0,len=strlen(line);            //starting addr
    char *ret;
    for(i=0; i<len; i++)
    {
       if(line[i]!=' ')                          // 28-37 copying label to structure
       {
           s->l[i] = line[i];
       }
       else{
    ...
    {   
        ...
        {
            s->s[k] = line[i];
            i++;                          //51 - 71 copy X, finnd x and copy to struct
            k++;
        }
    }
    else {
...
void pass1(struct state *s, int n){
    ...
    if(strcmp(s[0].ins, "START")==0)
     {  
        ...
        fscanf(f1, "%x", &st);                                       //79-89 open text file, keep count of location
        loc = st;
        s[1].add = loc;
        s[0].add =loc;
        fclose(f1);
    ...
    for(i=1; i<n; i++)
    {
        ...
        else if(strcmp(s[i].ins, "RESW")==0)
        {
            int x = atoi(s[i].s);
            loc = loc+(x*3);
        }                                                         //96-140 go through each instruction and increment accordingly 
...
void pass2(struct state *s, int n){
    ...
    char label[10];
    char symbol[10];
	for(i=1;i<n;i++)                                       //143-242 each instr is diff, handled in diff ways
	{   strcpy(label, s[i].l);
		...
		{
			s[i].oc = -0x1;
		}                                                     // copy struct to obj code with the help of optab & symtab
...
void createSymtab(struct state s[], int n){
    ...
    for(i=1; i<n; i++)                                            //244-255 scan state symbol put into symtab
    {
        if(strcmp(s[i].l,"****")!=0)
        {
            fprintf(f, "%x\t%s\n", s[i].add, s[i].l);
...
void disect2(char line[], struct opcode *o){
    ...
    for(; i<len; i++)
    {
        if(line[i]!=' ')                      //258-274 copying addr to opcode
        {o->val[j]=line[i];
         j++;
        }
        else
...
void createOptab(struct state s[], int n){
    ...
    while(fgets(line, 10, f1))                                   //copying all operands & their address into optab 
    {
        disect2(line, &O[m]);
        m++;
    }
...
void display(struct state s[], int n){
    ...
    else                                      //309-340 printing final table
    {
        printf("%x\t", s[i].add);
        printf("%s\t", s[i].l);
        printf("%s\t", s[i].ins);
...
void objectProgram(struct state S[], int n){
    ...
    printf("H^%s^%06x^%06x\n", S[0].l,S[0].add, len);             //printing object program 
    while(n>1)
    {
        int j=0, c=0, z=0;
        printf("T");
